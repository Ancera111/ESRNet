...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, kernel_size=3, n_channels=64):
        ...
        #每层之后加一个1x1的卷积
        self.conv = nn.Conv2d(in_channels=n_channels, out_channels=n_channels, kernel_size=1, stride=1)
    def forward(self, input):
        """
        前向传播.
...
class SRResNet(nn.Module):
    ...
    def __init__(self, large_kernel_size=3, small_kernel_size=3, n_channels=64, n_blocks=16, scaling_factor=4):
        ...
        assert scaling_factor in {2, 3, 4, 8}, "放大比例必须为 2、 3或 4!"
        ...
        # 放大通过子像素卷积模块实现, 每个模块放大两倍 scaling_factor=2/4
        # 若放大倍数为3 则scaling_factor=3
