...
def on_end_epoch(state):
    ...
    torch.save(model.state_dict(), r'F:\szy\old_computer\A.python\ESPCN\ESPCN-master\medicalepoch\epoch_%d_%d.pt' % (UPSCALE_FACTOR, state['epoch']))
...
if __name__ == "__main__":
    ...
    upscale_factor = 2
    num_epochs = 400
    # train = r'F:/szy/old_computer/A.python/ESPCN/ESPCN-master/data/train/'
    # val = r'F:/szy/old_computer/A.python/ESPCN/ESPCN-master/data/val'
    train = r'F:\szy\old_computer\A.python\SRRESNET\SRGAN\medicalimg\DATA\train'
    val = r'F:\szy\old_computer\A.python\SRRESNET\SRGAN\medicalimg\DATA\val'

    UPSCALE_FACTOR = upscale_factor
    NUM_EPOCHS = num_epochs

    train_set = DatasetFromFolder(train, upscale_factor=UPSCALE_FACTOR, input_transform=transforms.ToTensor(),
                                  target_transform=transforms.ToTensor())
    val_set = DatasetFromFolder(val, upscale_factor= UPSCALE_FACTOR, input_transform=transforms.ToTensor(),
                                target_transform=transforms.ToTensor())
